...
def gauss2(Y, n, m1, sig1, m2, sig2):
    ...
    for i in range(len(Y)):
        bruit = [scipy.stats.norm.pdf(Y[i], m1, sig1), scipy.stats.norm.pdf(Y[i], m2, sig2)]
        Mat_f.append(bruit)
    return Mat_f
...
def scaled_backward2(Mat_f,t,A):
    beta = []
    for i in range(t-1):
        beta.append([0, 0])
    beta.append([0.5, 0.5])
    ...
    for n in range(t-2, -1, -1):
        beta_recursive = np.zeros(2)
        for i in range(2):
            sum = beta[n+1][0]*A[i][0]*Mat_f[n+1][0] + beta[n+1][1]*A[i][1]*Mat_f[n+1][1]
            beta_recursive[i] = sum
        denominateur = beta_recursive[0] + beta_recursive[1]
        beta_recursive[0] = beta_recursive[0] / denominateur
        beta_recursive[1] = beta_recursive[1] / denominateur
        beta[n] = beta_recursive
    return beta
...
def BW(Mat_f,n,cl1,cl2,A,p10,p20):
    S = np.zeros(n)
    alfa = scaled_forward2(Mat_f,n,A,p10,p20)
    beta = scaled_backward2(Mat_f, n, A)
    for t in range(n):
